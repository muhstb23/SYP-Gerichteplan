...
import {players1} from "./mockdatenSpieler";

let players:IPlayer[] = players1;
...
router.get("/msroute", (req: Request, res: Response) => {
    ...
    if (sort === "des") {
        players.sort((a:IPlayer,b:IPlayer) => -1*(a.age - b.age));
    }
    else {
        players.sort((a: IPlayer, b: IPlayer) =>  (a.age - b.age));
    }
    res.send(players);
})
...
router.get("/filter",(req:Request, res)=> {
...
router.get("/name/:sucheName", (req:Request, res:Response) => {
    const sucheName:string   = req.params.sucheName;
    const player:IPlayer|undefined =
        players.find(p => p.lastname === sucheName);
    if (!player) {
...
router.get("/club/:club", (req: Request, res: Response) => {
    const clubParam = req.params.club.toLowerCase();
    console.log("get:clubParmLow=", clubParam);

    const result:IPlayer[] = players.filter( (p:IPlayer, index:number) => {
...
router.delete('/:id', (req, res) => {
    ...
    if (id > 0) {
        const newPlayers = players.filter ((p:IPlayer) => {
            return p.id != id;
        })
        deleted=true;
        players = [...newPlayers];
